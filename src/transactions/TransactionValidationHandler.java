    ...
    public boolean handle(Transaction tx) {
        ...
        if (from != null) {
            if (!from.getStatus().canBeSource()) {
                System.out.printf("[Validation] Source account %s status=%s => rejected%n", from.getId(), from.getStatusName());
                return false;
            }
        }
        ...
        if (to != null) {
            if (!to.getStatus().canReceive()) {
                System.out.printf("[Validation] Destination account %s status=%s => rejected%n", to.getId(), to.getStatusName());
                return false;
            }
        }

        // 4) Balance check for withdraw/transfer
        ...
        // pass to successor (boolean chain)
        if (successor != null) {
